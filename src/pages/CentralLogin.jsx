...
async function findUser(username, password) {
  const tablesToTry = ["private_users", "usuarios privados"];
  let lastError = null;
  ...
  throw lastError || new Error("No se pudo consultar la tabla de usuarios");
}
...
export default function CentralLogin() {
  ...
  useEffect(() => {
    ...
    if (user && ["superadmin", "concejal"].includes(user.role)) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.error("Error login central:", err);
    ...
    <main className="min-h-screen bg-[#050505] text-white flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md rounded-[30px] border border-cyan-400/15 bg-white/[0.04] px-6 py-7 shadow-[0_0_40px_rgba(0,255,255,0.08)]">
        ...
        <form onSubmit={handleLogin} className="mt-7 space-y-4">
          ...
          >
            {loading ? "INGRESANDO..." : "ENTRAR A CENTRAL"}
          </button>
        </form>
      </div>
    </main>
