...
async function findCoordinador(username, password) {
  ...
  throw lastError || new Error("No se pudo consultar la tabla de coordinadores");
}
...
export default function CoordinadorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { data } = await findCoordinador(username, password);
      ...
      console.error("Error login coordinador:", err);
      ...
      <div className="relative z-10 w-full max-w-md rounded-[30px] border border-cyan-400/15 bg-white/[0.04] px-6 py-7 shadow-[0_0_40px_rgba(0,255,255,0.08)]">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-2 text-sm text-white/55">
            Gestión de territorio, visitas y mensajería
          </p>
        </div>
